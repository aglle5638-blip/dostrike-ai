 ...
 *
 * X（Twitter）2段階投稿戦略:
 *   投稿① メイン: テキスト+画像のみ（URL なし）→ 低コスト
 *   投稿② リプライ: ①にぶら下げる形でURLのみ投稿 → さらに低コスト
 *
...
// ─────────────────────────────────────────────────────────────────────────────
// リプライ用テキスト（URLをぶら下げる投稿）
// ─────────────────────────────────────────────────────────────────────────────

/** メイン投稿へのリプライとして URL を貼る */
...
/** 日付シードでリプライテキストをローテーション選択 */
export function pickReplyText(dayIndex?: number): string {
  const idx = dayIndex !== undefined
    ? dayIndex % REPLY_TEMPLATES.length
    : Math.floor(Math.random() * REPLY_TEMPLATES.length);
  return REPLY_TEMPLATES[idx];
}

// ─────────────────────────────────────────────────────────────────────────────
// マーケティングセット定義（22種類・単一タイプ・URL なし）
// ─────────────────────────────────────────────────────────────────────────────
